...
export default function CustomProductsSection() {
    ...
    const handleSendWhatsApp = async () => {
        ...
        try {
            const res = await fetch('/api/send-whatsapp-custom', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    message: "Hola! Me interesa crear un producto personalizado. ¿Podrías ayudarme con más información?"
                })
            });
            ...
            if (res.ok) {
                ...
                if (data.method === 'link' && data.whatsappUrl) {
                    ...
                    toast.success("¡Mensaje enviado por WhatsApp! Pronto te contactaremos.");
                }
            } else {
                const errorData = await res.json().catch(() => ({}));
                toast.error(errorData.message || "Error al enviar el mensaje por WhatsApp.");
            }
        } catch (error) {
        ...
        <section className="flex flex-col lg:flex-row items-center justify-center gap-10 py-20 px-10 bg-gradient-to-br from-[#FFD166]/20 via-[#F6B78D]/30 to-[#E76F51]/20 backdrop-blur-lg border border-white/30 lg:w-screen rounded-2xl shadow-lg relative z-10">
            <div className="flex-1 max-w-2xl">
                <div className="text-center lg:text-left">
                    ...
                    <button
                        className="group px-12 py-5 rounded-3xl font-bold bg-gradient-to-r from-[#25D366] to-[#128C7E] text-white shadow-xl hover:scale-105 hover:shadow-2xl transition-all duration-300 relative overflow-hidden disabled:opacity-50 disabled:cursor-not-allowed"
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <span>{isSubmitting ? "Enviando..." : "Contactar por WhatsApp"}</span>
                        </div>
                        {/* Efecto de ondas */}
